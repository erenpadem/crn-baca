...
class ViewOrder extends ViewRecord
{
    ...
    protected function getHeaderActions(): array
    {
        ...
        $user = auth()->user();
        $isBayi = $user && $user->hasRole('bayi');
        $isMusteri = $user && $user->hasRole('musteri');
        $actions = [];

        if ($isBayi && $record->durum === Order::DURUM_TASLAK) {
        ...
        if ($isBayi && Order::bayiKarsiTeklifEdebilir($record)) {
        ...
        if ($isMusteri && $record->durum === Order::DURUM_MUSTERI_ONAYI_BEKLIYOR) {
